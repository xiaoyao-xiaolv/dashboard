// 参考链接：https://squarechip.github.io/timeline/#horizontal-demo

const timeline = (collection, options?: any) => {
  const timelines = [];
  ...
  function isElementInViewport(el: any, triggerPosition: any) {
    ...
    const defaultTrigger: any = defaultSettings.verticalTrigger.defaultValue.match(/(\d*\.?\d*)(.*)/);
    let triggerUnit :any= triggerPosition.unit;
    let triggerValue: any = triggerPosition.value;
    let trigger: any = windowHeight;
