...
export default class Visual extends WynVisual {
  ...
  public update(updateOptions: VisualNS.IVisualUpdateOptions) {
    ...
    this.hasLink = !!(dataView && dataView.plain.profile.link.values.length);
    ...
    if (this.isMock) {
      ...
      this.linkName = this.hasLink && plainData.profile.link.values[0].display || '';
  ...
  public render() {
    ...
    for (let i = 0; i < rotateY.length; i++) {
      ...
      let figureElement = $("<div>").attr('class', 'figure frame')
        .attr('selectedId',() => this.isMock ? -1 : i)
